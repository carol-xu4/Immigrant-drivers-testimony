...
# ACS data -----------------------------------------------------------------
ddi_acs = read_ipums_ddi("data/input/usa_00024.xml")
...
acs = acs %>%
  select(
    year, serial, pernum, cluster, strata, perwt, hhwt, statefip,
    sex, age, race, hispan, marst, gq,
    citizen, yrimmig, bpl, bpld, vetstat,
    incss, incwelfr, incsupp,
    classwkr, classwkrd, relate, related,
    hinscaid, hinscare, 
    occ2010, ind, tranwork, trantime, vehicles)

# restrict to non gq for 2006+ (to match 2004/2005 non-GQ dataset)
# also restrcited to age 16+ to reduce data size
acs = acs %>%
  filter(gq %in% c(1, 2)) %>%
  filter(age >= 16)
...
acs = acs %>%
    select(-c(immigrant, legal, good, slegal, good1, hlegal, undercount))

fwrite(acs, "data/output/acs_drivers.csv")
